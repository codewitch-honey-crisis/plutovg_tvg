...
static result_t tvg_parse_fill_header(tvg_context_t* ctx,int kind,tvg_fill_header_t* out_header) {
    uint32_t u32;
...
static result_t tvg_parse_line_header(tvg_context_t* ctx,int kind,tvg_line_header_t* out_header) {
    uint32_t u32;
    result_t res = tvg_read_u32(ctx,&u32);
    if(res!=TVG_SUCCESS) {
        return res;
...
static result_t tvg_parse_outline_fill_header(tvg_context_t* ctx,int kind,tvg_outline_fill_header_t* out_header) {
    uint32_t u32;
    uint8_t d;
    ...
    for(size_t j=0;j<size;++j) {
        ...
        switch(TVG_PATH_CMD_INDEX(d)) {
            ...
            case TVG_PATH_ARC_CIRCLE: {
                ...
                plutovg_canvas_arc_to(ctx->cvs,radius,radius,0,TVG_ARC_LARGE(d),1-TVG_ARC_SWEEP(d),pt.x,pt.y);
                cur = pt;
            }
            break;
            case TVG_PATH_ARC_ELLIPSE: {
                ...
                plutovg_canvas_arc_to(ctx->cvs,radius_x,radius_y,rotation* (TVG_PI/180.0f),TVG_ARC_LARGE(d),1-TVG_ARC_SWEEP(d),pt.x,pt.y);
                cur = pt;
            }
            break;
            case TVG_PATH_CLOSE:
...
int main(int argc, char* argv[])
{
    ...
    plutovg_surface_t* surface = plutovg_surface_create((int)(w*scale),(int)(h*scale));
